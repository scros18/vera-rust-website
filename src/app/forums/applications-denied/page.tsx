...
export default function DeniedApplicationsPage() {
  const { isAuthenticated } = useAuth();
  const router = useRouter();
  const [posts, setPosts] = useState<any[]>([]);

  useEffect(() => {
    const deniedPosts = getForumPostsByCategory('applications-denied');
    setPosts(deniedPosts);
  }, []);
    ...
    <div className="min-h-screen bg-[#0a0a0a] relative overflow-hidden">
          ...
          <div className="flex items-center gap-6 mb-8">
            <div 
              ...
              style={{ 
                ...
                boxShadow: '0 10px 30px #ef444430'
              }}
            >
              ❌
            </div>
            <div>
              ...
              <p className="text-gray-400 text-sm md:text-base">
                View denied staff applications
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-8">
        ...
        <div className="mb-8 flex items-center gap-2 text-sm">
          ...
          <span className="text-gray-600">/</span>
          <span className="text-red-500 uppercase font-semibold tracking-wide">Denied Applications</span>
        </div>
        ...
        <div className="bg-[#0f0f0f] border border-[#1f1f1f] rounded-lg overflow-hidden">
          <div className="bg-[#0a0a0a] border-b border-[#1f1f1f] px-6 py-4">
            <h3 className="font-bold text-white uppercase tracking-wide">
              Denied Applications ({posts.length})
            </h3>
          </div>

          {posts.length === 0 ? (
            <div className="px-6 py-12 text-center">
              <p className="text-gray-500">No denied applications yet.</p>
            </div>
          ) : (
            <div className="divide-y divide-[#1f1f1f]">
              {posts.map((post) => (
                <Link
                  key={post.id}
                  href={`/forums/applications-denied/${post.id}`}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-3 text-xs text-gray-500">
                        ...
                        <span>•</span>
                        <span>Denied on {new Date(post.updatedAt).toLocaleDateString()}</span>
                      </div>
                    </div>
